...
async def upload_contracts(files: List[UploadFile] = File(...)):
    ...
    for file in files:
        # 验证文件类型
        if file.filename and not any(file.filename.endswith(ext) for ext in LongContextConfig.FILE_PROCESSING["supported_formats"]):
...
async def review_batch_contracts(
    ...
    # 设置默认工作流
    if not workflow_id:
        workflow_id = LongContextConfig.DIFY_WORKFLOWS["batch_contract_review"]["workflow_id"]
...
async def estimate_processing_cost(file_paths: List[str], model_name: Optional[str] = None):
    """估算处理成本"""
    total_tokens = 0
    file_info = []
...
async def process_batch_contracts(task_id: str, file_paths: List[str], workflow_id: str):
    """处理批量合同的后台任务"""
    try:
        task_status[task_id]["progress"] = 10
